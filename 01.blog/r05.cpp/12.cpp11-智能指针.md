...
&emsp;&emsp;weak_ptr看起来更像shared_ptr的附属品，它从shared_ptr衍生，但不会控制所指向对象的生命周期；和shared_ptr共享一个引用计数。
...
int main() {
    ...
    std::weak_ptr<int> wp = sp1; // shared_ptr 构造 weak_ptr
...
C++17引入返回一个指向当前对象的智能指针，伪代码```std::shared_ptr<T> shared_from_this() { return std::shared_ptr<T>(weak_ptr); }```。

...
虚假唤醒是指线程在没有收到notify通知的情况下被唤醒，和操作系统相关。所以被唤醒后，需要检查一下需要处理的数据
...
&emsp;&emsp;```std::recursive_mutex```：递归互斥量，可重入，不带超时功能 （同一个线程，第二次加锁，不阻塞）  
...
普通锁，如果当前锁已经被持有(lock后，还没有unlock)，第二次lock的时候，会阻塞。  
如果同线程二次上锁，会发生死锁。
...
如果其他线程已经持有锁，本线程lock的时候，会阻塞等待锁内部的计数归0(锁被释放)。此处逻辑和mutex相同
如果本线程持有锁，owner_tid = current_tid，lock_count。
...
int main() {
    ...
    std::thread t([&] {
        ...
        // 设置promise的值, set_value执行后，另外一个线程继续运行（之前在fut.get()处阻塞）
        prom.set_value(10);
    });
    t.detach();
...
<div align="right"><font color="#999"><small>📅 Created: 2024-04-02  ·  ✏️ Update: 2026-03-06</small></font></div>

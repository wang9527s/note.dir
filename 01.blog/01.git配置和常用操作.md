...
- [七、其他操作](#七其他操作)
  ...
  - [5. 下载 Git 仓库的某个文件夹](#5-下载-git-仓库的某个文件夹)
...
- [九、commit id 和 change id](#九commit-id-和-change-id)
  - [背景问题](#背景问题)
  - [commit id](#commit-id)
  - [Change-Id](#change-id)
...
PS: git note 有类似效果，都可以在不改变commit hash的情况下附加信息。
...
git log --oneline -- ./obs/img/1.png
git log --follow  -- ./obs/img/1.png
...
今天遇到一个奇怪的问题，mac上vs code的终端，pod install 一直执行失败，账号的密钥没修改过，配置也没问题。

最后发现是 GIT_SSH_COMMAND 设置的不是默认密钥。（我新打开的，不是分割出来的终端环境也有问题。）  
新打开的其他终端没问题。

```bash
➜  LyraWatchCppDemo echo $GIT_SSH_COMMAND
ssh -i ~/.ssh/id_rsa_wb

# 取消设置和设置环境变量
➜  LyraWatchCppDemo unset GIT_SSH_COMMAND
➜  LyraWatchCppDemo export GIT_SSH_COMMAND="ssh -i  ~/.ssh/id_rsa"  
...
### 11. 服务端的git
   ...
   一般设置一个裸仓库作为server端（中转仓库），否则接受push的时候会有问题。  
   在服务端执行```git init --bare note.dir```后，client使用```git clone ssh://username@ip:/path/note.dir```即可clone仓库。

## 九、commit id 和 change id

### 背景问题

  发现不同机器上，同一个提交看起来对应的 commit id 不同；思考commit id、change id的问题。

### commit id

+ commit id 是由 commit 对象内容计算出来的哈希值。
...
<div align="right"><font color="#999"><small>📅 Created: 2022-08-17  ·  ✏️ Update: 2026-06-29</small></font></div>

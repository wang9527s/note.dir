...
- **Rebase**：在 master 最新处创建新的 dev 分支，将 dev 的提交重新应用到新的分支上。
- **Merge**：合并 dev 分支到 master 上，生成新的提交 H'。

```bash
# Rebase 操作
git rebase master
git rebase master -i                # -i 编辑模式，可删除不需要 rebase 的提交
...
```bash
# --no-ff 强制创建一个merge提交记录，适用于 main 分支和 dev 分支没有分叉（即 main 分支没有新的提交）
git merge --no-ff dev
```
...
Host github.com
    User git
    ProxyCommand connect -S 172.27.80.1:10810 %h %p
```

备注：需要安装``` connect-proxy```
	
+ http代理

**连接本机成功，wsl中连接物理机代理失败**

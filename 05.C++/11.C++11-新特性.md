...
&emsp;&emsp;那什么时候编译器会进行返回值优化呢?

&emsp;&emsp;&emsp;&emsp;return的值类型与函数的返回值类型相同
&emsp;&emsp;&emsp;&emsp;return的是一个局部对象
...
假如线程函数执行时间较长，并且没有调用joion和detach；在线程对象的生命周期结束，释放资源的时候可能会发生未知错误。
